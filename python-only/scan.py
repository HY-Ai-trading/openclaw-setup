...
# 데이터 기반 고변동성·고승률 종목 14개
# 제거: 저변동성 금융/통신/가전, EV음수 종목(GS 33%, 현대제철 45%, 두산에너빌 0%, KT 0%)
NAME_MAP = {
    "084850":"아이티엠반도체",                          # 100% WR
    "086520":"에코프로",  "247540":"에코프로비엠", "003670":"포스코퓨처엠",
    "064350":"현대로템",  "042660":"한화오션",   "272210":"한화시스템",
    "329180":"HD현대중공업","011200":"HMM",         # HMM 50% WR
    "015760":"한국전력",                                # 50% WR
    "003490":"대한항공",                                # 67% WR
    "000880":"한화",
    "316140":"우리금융",  "032640":"LG유플러스",   # 저가 (주수 극대화)
}

# 데이터 기반 재편 — EV>0 or 고변동성 테마 14종목
# 제거된 16종목: 삼성전자/기아(저변동), KB금융/신한/하나/메리츠(저변동 금융),
#   두산에너빌(0%WR), 삼성중공업(0%WR), LG전자/SK텔레콤/KT(0%WR/저변동),
#   포스코인터내셔널/현대건설/현대제철(EV-)/GS(33%WR)/두산/에쓰오일
WATCHLIST = [
    # 반도체 (1) — 100% 승률
    "084850",  # 아이티엠반도체  ~8만  ★WR100% EV+5.4%

    # 2차전지 (3) — 고변동성 테마
    "086520",  # 에코프로        ~6만  (3~7% daily)
    "247540",  # 에코프로비엠    ~5만  (고변동)
    "003670",  # 포스코퓨처엠   ~20만  (고변동)

    # 방산/조선/해운 (5) — 최고 단타 적합 테마
    "064350",  # 현대로템        ~5만  (방산 ★)
    "042660",  # 한화오션        ~6만  (조선 ★)
    "272210",  # 한화시스템      ~2만  WR50%
    "329180",  # HD현대중공업   ~20만  (조선 ★)
    "011200",  # HMM            ~2만  WR50% EV+0.01% ★

    # 에너지 (1)
    "015760",  # 한국전력        ~3만  WR50%

    # 항공 (1) — 최고 승률
    "003490",  # 대한항공        ~2만  ★WR67% EV+1.9%

    # 그룹 지주 (1)
    "000880",  # 한화            ~9만  (한화오션·한화시스템 모회사)

    # 저가 (2) — 94K 자본 주수 극대화
    "316140",  # 우리금융        ~3만  (6주 가능)
    "032640",  # LG유플러스      ~1만  (9주 가능)
]  # 총 14개 — 집중 포트폴리오
...
def main():
    ...
    if k_rsi < 45 and not bear_market:
        ...
        else:  # 과매도 반등장 — k_dead 이미 +3 적용됐으므로 추가 없음
            print(f"📊 KODEX RSI{k_rsi:.0f} 과매도 반등{k_chg:+.1f}% — 데드크로스 기준 유지")
